...
class Extra_Filters_Tests(Base_TestCase):
    def test_format_mentions_filter_user_exists(self):
        ...
        formatted_message = pulsifi_extras.format_mentions(f"{message[:len(message) // 2]}@{user.username} {message[len(message) // 2:]}")
    ...
    def test_format_mentions_filter_invalid_username(self):
        ...
        formatted_message = pulsifi_extras.format_mentions(f"{message[:len(message) // 6]}@$$hi {message[len(message) // 6:(2 * len(message)) // 6]}@admin {message[(2 * len(message)) // 6:(3 * len(message)) // 6]}@https {message[(3 * len(message)) // 6:(4 * len(message)) // 6]}@abuse {message[(4 * len(message)) // 6:(5 * len(message)) // 6]}@docs {message[(5 * len(message)) // 6:]}")
    ...
    def test_model_meta_tag(self):
        ...
        for model_name in pulsifi_tests_utils.GENERATABLE_MODELS_NAMES:
            ...
            self.assertEqual(
                ...
                "".join([line.strip() for line in Template("{% load pulsifi_extras %}\n{% model_meta obj as obj_meta %}\n{{ obj_meta.model_name }}").render(Context({"obj": obj})).splitlines()])
            )
            self.assertEqual(
                obj._meta.verbose_name,
                "".join([line.strip() for line in Template("{% load pulsifi_extras %}\n{% model_meta obj as obj_meta %}\n{{ obj_meta.verbose_name }}").render(Context({"obj": obj})).splitlines()])
            )
